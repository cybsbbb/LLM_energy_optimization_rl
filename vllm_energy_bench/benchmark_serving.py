...
def sample_sharegpt_requests(
    ...
    for i in range(len(dataset)):
        ...
        current_output_len = fixed_output_len if fixed_output_len is not None else len(tokenizer(completion).input_ids)

        samples.append((truncated_prompt, len(prompt_token_ids), current_output_len, None))

    return samples

def sample_squad_v2_requests(
    num_requests: int,
    tokenizer: PreTrainedTokenizerBase,
    dataset_split: str = "train"
) -> List[Tuple[str, int, int, None]]:
    dataset = load_dataset("squad_v2", split=dataset_split)
    ...
    for example in dataset:
        ...
        if example['answers']['text']:
            ...
            output_token_ids = tokenizer(answer).input_ids
...
def sample_eli5_requests(
    num_requests: int,
    tokenizer: PreTrainedTokenizerBase,
    dataset_split: str = "train"
) -> List[Tuple[str, int, int, None]]:
    dataset = load_dataset("sentence-transformers/eli5", split=dataset_split)
    dataset = dataset.shuffle(seed=42)
...
def sample_wnut_requests(
    num_requests: int,
    tokenizer: PreTrainedTokenizerBase,
    dataset_splits: List[str] = ["train", "test", "validation"]
) -> List[Tuple[str, int, int, None]]:
    ...
    for split in dataset_splits:
        ...
        for example in dataset[split]:
            ...
            prompt = f"Sentence: {sentence}\nCan you identify the {entity} mentioned in the above sentence?"
            prompt_token_ids = tokenizer(prompt).input_ids
            prompt_len = len(prompt_token_ids)

            entities = example["ner_tags"]
            output_text = " ".join(
                [example["tokens"][i] for i in range(len(entities)) if entities[i] != 0]
            )
            output_token_ids = tokenizer(output_text).input_ids
            output_len = len(output_token_ids) if output_text else 50
...
def sample_imdb_requests(
    ...
    question_templates = [
        "Based on this review, judge if the user liked this movie or not?",
        "Please identify if the review is positive or negative?",
        "Based on this review, should we recommend this movie to other users with similar tastes?"
    ]
    ...
    for example in dataset:
        ...
        requests.append((truncated_prompt, prompt_len, output_len, None))
...
def main(args: argparse.Namespace):
    ...
    if args.dataset is not None:
        ...
        input_requests = sample_squad_v2_requests(
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            dataset_split="train"
        )
        ...
        input_requests = sample_eli5_requests(
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            dataset_split="train"
        )
        
    elif args.dataset_name == "wnut":
        input_requests = sample_wnut_requests(
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
        ...
        input_requests = sample_imdb_requests(
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            dataset_split="train",
            input_len=args.fixed_input_len
        ...
        input_requests = sample_mix_requests(
            dataset_path=args.dataset_path,
            num_requests=args.num_prompts
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Truncate input length to a fixed number of tokens."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="sharegpt",
        choices=["sharegpt", "burstgpt", "sonnet", "random", "hf", "squad_v2", "eli5", "imdb", "wnut", "mix"],
