...
class LLMDataCenterEnv(gym.Env):
    ...
    Fixed to handle continuous action algorithms properly.
    """
    ...
    def __init__(self,
        ...
        self.energy_price = caiso_data_20250421
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict] = None) -> Tuple[Dict, Dict]:
        ...
        # Action statistics - initialize with all possible actions
    ...
    def step(self, action: int) -> Tuple[Dict, float, bool, bool, Dict]:
        ...
        # Safely increment action count
        if action in self.action_counts:
            self.action_counts[action] += 1
        else:
            print(f"Warning: Unexpected action {action}, clipping to valid range")
            action = np.clip(action, 0, self.n_actions - 1)
            self.action_counts[action] += 1
