...
class DiscreteToBoxWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        discrete_action = int((action[0] + 1) * self.n_actions / 2)
...
class LLMDataCenterAgent:
    ...
    def create_model(self, env=None):
        ...
        if self.algorithm == "PPO":
            ...
            n_actions = wrapped_env.action_space.shape[-1]
    ...
    def load(self, path: str, env=None):
        ...
        if self.algorithm == "PPO":
            ...
            wrapped_env = DiscreteToBoxWrapper(env)
            self.model = SAC.load(path, env=wrapped_env)
        elif self.algorithm == "TD3":
            wrapped_env = DiscreteToBoxWrapper(env)
            self.model = TD3.load(path, env=wrapped_env)
